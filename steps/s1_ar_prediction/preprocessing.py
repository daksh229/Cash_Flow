"""
S1 AR Collections Prediction - Preprocessing
==============================================
Takes merged input from input_format, derives the target variable
(days_to_pay), encodes features, and splits into train/test sets.

All encoding rules and split settings are read from config.
"""
...
# Logger
# ---------------------------------------------------------------------------
logger = logging.getLogger("s1.preprocessing")
...
def run(merged_df, config=None):
    """
    Preprocess merged data for S1 model training.
    ...
    logger.info("=" * 60)
    logger.info("S1 PREPROCESSING - START")
    logger.info("=" * 60)
    ...
    # ------------------------------------------------------------------
    # 1. Derive target variable from config
    # ------------------------------------------------------------------
    target_cfg = config.get("target", {})
    target_col = target_cfg.get("column", "days_to_pay")
    ...
    ref_type = derive_cfg.get("reference_type", "AR")

    payments = pd.read_csv(DATA_DIR / payments_file)
    payments["payment_date"] = pd.to_datetime(payments["payment_date"])

    # Get payments of the specified type, take first per invoice
    ...
    df[target_col] = (df["payment_date"] - df["invoice_date"]).dt.days

    # Keep only rows with known target
    paid_count = df[target_col].notna().sum()
    total_count = len(df)
    logger.info(
        "  Invoices with payment: %d / %d (%.1f%%)",
    ...
    min_days = outlier_cfg.get("min_days", 0)
    max_days = outlier_cfg.get("max_days", 365)
    before = len(df)
    ...
    for col_name, enc_info in encoding_cfg.get("ordinal", {}).items():
        ...
        if col_name in df.columns:
            df[output_col] = df[col_name].map(mapping).fillna(
                list(mapping.values())[len(mapping) // 2]  # default to middle value
            )
    ...
    logger.info("  Using %d / %d configured features", len(available_features), len(feature_columns))
    ...
    sort_col = split_cfg.get("sort_column", "invoice_date")
    ...
    meta_cols = ["invoice_id", "customer_id", "invoice_date", "due_date", "invoice_amount"]
    ...
    # Also keep full dataset metadata for generating predictions on all data
    ...
    logger.info("=" * 60)
    logger.info("S1 PREPROCESSING - COMPLETE")
    logger.info("=" * 60)
